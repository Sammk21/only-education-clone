...
import { FilterParams, SearchParamsProps, UniversitiesData, Universitylist } from "@/types/types";
...
import { examsQuery, indianStatesQuery, ownershipQuery, rankingQuery, streasmQuery } from "@/app/data/quries/uniList-query";
...
export default async function UniversitiesList({
  ...
  const [ownership, indianStates, exams, streams, ranking, data, user] = await Promise.all([
    getStrapiData(ownershipQuery),
    getStrapiData(indianStatesQuery),
    getStrapiData(examsQuery),
    getStrapiData(streasmQuery),
...
// export default async function UniversitiesList({
//   searchParams,params
// }:{searchParams: Readonly<SearchParamsProps>, params:{stream:string}}) {
