...
const DocumentRquired = ({ data }: Props) => {
  ...
  return (
    <section className=" container  border-y  text-dark  dark:text-light py-12 mt-8 ">
      <div>
        <h4 className=" text-4xl mb-4  font-medium">
        ...
        <ul className=" space-y-1 text-gray-500 list-disc list-inside dark:text-gray-400">
          {data.documents.map((item) => (
            <li key={item.id} className="flex ">
              ...
              <p>
                <strong className="mr-1 text-dark"> {item.title}</strong>
                {item.description}
              </p>
            </li>
          ))}
