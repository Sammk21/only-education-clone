...
const RankingUniversityPage = async ({ slug }: { slug: string }) => {
  const data: Data = await getStrapiData(
    `/api/universities?filters[slug][$eq]=${slug}&fields[0]=title&populate[rankingCourses][populate][course][populate]=true`
  );
  ...
  console.dir(streamData, { depth: null });
  //   const isChampion =
  //   item.rankingNumber >= 1 && item.rankingNumber <= 25;
  //     // Parse the date and extract the year
  //  const rankingYear = item.rankingYear ? new Date(item.rankingYear).getFullYear() : '';
                    ...
                    <tr key={item.id} className="bg-white border-b">
                      ...
                      <td className="px-6 py-4">
                        ...
                        {/* <div className="flex items-center space-x-2">
                            ...
                            <strong
                              className={
                                isChampion ? "text-blue-500 font-bold" : ""
                              }
                            >
                              {item.rankingNumber}
                            </strong>
