    ...
    <li>
      <NavigationMenuLink asChild>
        <a
          ref={ref}
          className={cn(
            "block select-none space-y-1  p-3 leading-none no-underline outline-none transition-colors hover:bg-accent/20  duration-200 rounded-2xl hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground",
            className
          )}
          {...props}
        >
...
const SubMenuSection = ({ title, items }: { title: string, items: SubmenuItem[] }) => {
  return (
    <div className="mb-4 ">
...
const renderSubMenu = (submenu: SubmenuItem[]) => {
  return submenu.map((section) => (
    <SubMenuSection key={section.id} title={section.title} items={section.submenu || []} />
  ))
}
